...
<body>
    <header>
        ...
        <div class="button-container">
            ...
            <button class="dark-mode-toggle button">Toggle Dark Mode</button>
        </div>
    </header>

    <main>
        <section>
            <h1>ⴰⵙⵏⵎ ⵏ ⵜⵉⵔⵔⴰ ⵏ ⵜⵎⴰⵣⵉⵖⵜ</h1>
            <textarea id="inputText" placeholder="ⴰⵔⴰ ⴰⴹⵕⵉⵚ ⵏⵏⴽ ⴷⴰ..."></textarea>
            <div class="button-container">
                <button id="checkSpelling" data-i18n="check-spelling">ⵙⵏⵎ ⵜⵉⵔⵔⴰ</button>
                <button id="clearInput" data-i18n="clear-input">ⵙⴼⴹ ⴰⵔⵓ</button>
                <button id="pasteInput" data-i18n="paste-input">ⵍⵙⵇ ⵙⵖ ⵓⵙⵍⵓⵖⵓ</button>
                <button id="exportResults" data-i18n="export-results">ⵙⵓⴼⵖ ⵉⵙⵓⴼⵖⵏ</button>
            </div>
            <div id="resultText"></div>
            <div class="button-container">
                <button id="copyResult" data-i18n="copy-result">ⵙⵙⵏⵖⵍ ⴰⵙⵓⴼⵖ</button>
            </div>
        </section>
    </main>

    <footer>
        <p data-i18n="copyright">© 2024 AmTools. All rights reserved.</p>
    </footer>
    ...
    <div id="myModal" class="modal">
        <div class="modal-content">
            ...
            <p data-i18n="similar-words">ⴰⵡⴰⵍⵏ ⵉⵎⵇⵔⴰⵏⵏ:</p>
            <p id="modalContent"></p>
            ...
            <button id="addToDictionary" data-i18n="add-to-dictionary">ⵔⵏⵓ ⵖⵔ ⵓⵎⴰⵡⴰⵍ</button>
        </div>
    </div>
...
const translations = {
  en: {
    ...
    "copyright": "© 2024 AmTools. All rights reserved.",
  ...
  ar: {
    "tools-title": "أدواتي",
    "check-spelling": "التحقق الإملائي",
    "clear-input": "مسح الإدخال",
    "paste-input": "لصق من الحافظة",
    "export-results": "تصدير النتائج",
    "copy-result": "نسخ النتيجة",
    "copyright": "© 2024 أدواتي. جميع الحقوق محفوظة.",
  ...
  fr: {
    ...
    "clear-input": "Effacer le contenu",
    "paste-input": "Coller depuis le presse-papiers",
    "export-results": "Exporter les résultats",
    "copy-result": "Copier le résultat",
    "copyright": "© 2024 AmTools. Tous droits réservés.",
  ...
  zgh: {
    "tools-title": "ⵉⵎⴰⵙⵙⵏ",
    "check-spelling": "ⵙⵏⵎ ⵜⵉⵔⵔⴰ",
    "clear-input": "ⵙⴼⴹ ⴰⵔⵓ",
    "paste-input": "ⵍⵙⵇ ⵙⵖ ⵓⵙⵍⵓⵖⵓ",
    "export-results": "ⵙⵓⴼⵖ ⵉⵙⵓⴼⵖⵏ",
    "copy-result": "ⵙⵙⵏⵖⵍ ⴰⵙⵓⴼⵖ",
    "copyright": "© 2024 ⵉⵎⴰⵙⵙⵏ. ⴽⵓⵍ ⵜⴰⵎⴰⵎⴽⵉⵏ ⵃⴹⴰⵏ.",
    "similar-words": "ⴰⵡⴰⵍⵏ ⵉⵎⵇⵔⴰⵏⵏ:",
    "replace-word": "ⵙⵎⴰⵜⵜⵉ",
    "add-to-dictionary": "ⵔⵏⵓ ⵖⵔ ⵓⵎⴰⵡⴰⵍ"
  }
};
...
languageSelect.value = initialLanguage;
        ...
        loadDictionary('user_dict.txt', userDictionary);

        function saveToUserDictionary(word) {
            fetch('user_dict.txt', {
                method: 'POST',
            ...
            .then(response => {
                if (response.ok) {
                    ...
                    console.log(`Added "${word}" to user dictionary`);
                    alert(`"${word}" ⵉⵜⵜⵓⵔⵏⴰ ⵖⵔ ⵓⵎⴰⵡⴰⵍ ⵏⵏⴽ ⴰⵡⴷⵎⴰⵡⵏ.`);
                }
            })
            .catch(error => {
                console.error('Error saving to user dictionary:', error);
                alert('ⵓⵔ ⵉⵍⵍⵉ ⵓⵔⵏⵓ ⵏ ⵡⴰⵡⴰⵍ ⵖⵔ ⵓⵎⴰⵡⴰⵍ ⵏⵏⴽ ⴰⵡⴷⵎⴰⵡⵏ. ⵎⴽ ⵜⵓⴼⵉⴷ ⴰⵔⵎ ⴷⴰⵖ.');
            });
        }
...
function updateModalContent() {
        ...
        function handleWordClick(event) {
            ...
            const similarWords = findSimilarWords(currentWordText, dictionary).concat(findSimilarWords(currentWordText, userDictionary));
        ...
        function replaceWord(newWord) {
            ...
            if (!dictionary.has(newWord) && !userDictionary.has(newWord)) {
                saveToUserDictionary(newWord);
            }
        }
        ...
        document.getElementById('checkSpelling').addEventListener('click', function() {
            ...
            words.forEach(word => {
                ...
                if (cleanWord.trim().length > 0 && !dictionary.has(cleanWord) && !userDictionary.has(cleanWord)) {
       ...
       document.getElementById('replaceWord').addEventListener('click', function() {
        ...
        document.getElementById('addToDictionary').addEventListener('click', function() {
            const newWord = document.getElementById('editWord').value;
            saveToUserDictionary(newWord);
        });
